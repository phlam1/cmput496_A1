...
"""
ToDos:
- Need to change def final_score(self,komi):
...
class GoBoard(object):
        ...
        for y in range(1,self.size+1,1):
            for x in range(1,self.size+1,1):
                ...
                if self.get_color(point)!=EMPTY:
                    continue
                """
                Removing constraint on playing on eye
    ...
    def _play_move(self,point, color):
        ...
        single_captures = []
        cap_inds = None
        cap_flag = False #Added capture flag
        ...
        #Added capture constraint
        if cap_flag == True:
        	self.board[point] = EMPTY
        	c = self._point_to_coord(point)
        	msg = "capture"
        	#raise ValueError("capture") # this wrecks legal_moves command
        	return False, msg
